...
| `Admin/SecureConnectionEndpoints:GET:/api/v{version:apiVersion}/admin/connections/` | P0 | supported | /connections (ConnectionListPage) and /operator/publishing (PublishingWorkspace) | Powers the Connections list and PublishingWorkspace connection selector. |
...
| `Admin/SecureConnectionEndpoints:POST:/api/v{version:apiVersion}/admin/connections/{id:guid}/test` | P0 | supported | /connections/{id} (ConnectionDetailPage) and /operator/publishing (PublishingWorkspace) | Connection detail and PublishingWorkspace expose health test actions. |
| `Admin/LayerPublishingEndpoints:GET:/api/v{version:apiVersion}/admin/connections/{id}/layers/` | P0 | supported | /layers, /connections/{id} (LayerListPage / ConnectionDetailPage), and /operator/publishing (PublishingWorkspace) | Layer pages and PublishingWorkspace list published layers for a selected connection/service. |
| `Admin/LayerPublishingEndpoints:POST:/api/v{version:apiVersion}/admin/connections/{id}/layers/` | P0 | supported | /connections/{id}/publish (PublishLayerPage) and /operator/publishing (PublishingWorkspace) | PublishLayerPage and PublishingWorkspace submit layer publish requests. |
| `Admin/LayerPublishingEndpoints:PUT:/api/v{version:apiVersion}/admin/connections/{id}/layers/enabled` | P0 | supported | /layers (LayerListPage) and /operator/publishing (PublishingWorkspace) | LayerListPage and PublishingWorkspace bulk enable or unpublish service layers. |
| `Admin/LayerPublishingEndpoints:PUT:/api/v{version:apiVersion}/admin/connections/{id}/layers/{layerId:int}/enabled` | P0 | supported | /layers, /connections/{id} (LayerListPage / ConnectionDetailPage), and /operator/publishing (PublishingWorkspace) | Layer tables and PublishingWorkspace expose per-layer publish/unpublish actions. |
| `Admin/AdminEndpoints:GET:/api/v{version:apiVersion}/admin/connections/{id}/tables` | P0 | supported | /connections/{id} (ConnectionDetailPage), /connections/{id}/publish (PublishLayerPage), and /operator/publishing (PublishingWorkspace) | Discovery tables feed the connection detail, publish-layer, and guided publishing workspace workflows. |
...
| `Admin/DeployControlEndpoints:GET:/api/v{version:apiVersion}/admin/deploy/preflight` | P0 | supported | /deploy (DeployControlPage) and /operator/operations (OperationsConsole) | Deploy control and operations console run diagnostics with includeDiagnostics=true. |
...
| `Admin/AdminManifestDriftEndpoints:GET:/api/v{version:apiVersion}/admin/manifest/drift` | P0 | supported | /operator/publishing (PublishingWorkspace) and /operator/operations (OperationsConsole) | Publishing workspace and operations console show manifest baseline drift resources alongside service state. |
| `Admin/AdminManifestApprovalEndpoints:GET:/api/v{version:apiVersion}/admin/manifest/pending/` | P0 | supported | /operator/publishing (PublishingWorkspace) and /operator/operations (OperationsConsole) | Publishing workspace and operations console render pending manifest approvals. |
...
| `Admin/AdminManifestDriftEndpoints:GET:/api/v{version:apiVersion}/admin/manifest/versions` | P0 | supported | /operator/publishing (PublishingWorkspace) and /operator/operations (OperationsConsole) | Publishing workspace and operations console load recent manifest versions for release evidence. |
...
| `Admin/ObservabilityEndpoints:GET:/api/v{version:apiVersion}/admin/observability/errors` | P0 | supported | /observability (ObservabilityPage) and /operator/operations (OperationsConsole) | Errors tab and operations console render the recent-error buffer. |
| `Admin/ObservabilityEndpoints:GET:/api/v{version:apiVersion}/admin/observability/migrations` | P0 | supported | /observability (ObservabilityPage) and /operator/operations (OperationsConsole) | Migrations tab and operations console render lifecycle and plan status. |
| `Admin/ObservabilityEndpoints:GET:/api/v{version:apiVersion}/admin/observability/telemetry` | P0 | supported | /observability (ObservabilityPage) and /operator/operations (OperationsConsole) | Telemetry tab and operations console render tracing and OTLP status. |
...
| `Admin/ServiceSettingsEndpoints:GET:/api/v{version:apiVersion}/admin/services/` | P0 | supported | /services (ServiceListPage) and /operator/publishing (PublishingWorkspace) | ServiceListPage renders service summaries; PublishingWorkspace selects a service target. |
...
| `Admin/ServiceSettingsEndpoints:PUT:/api/v{version:apiVersion}/admin/services/{serviceName}/protocols` | P0 | supported | /services/{serviceName}/settings (ServiceSettingsPage) and /operator/publishing (PublishingWorkspace) | Protocols tab and PublishingWorkspace update the enabled protocol list. |
| `Admin/ServiceSettingsEndpoints:GET:/api/v{version:apiVersion}/admin/services/{serviceName}/settings` | P0 | supported | /services/{serviceName}/settings (ServiceSettingsPage) and /operator/publishing (PublishingWorkspace) | ServiceSettingsPage and PublishingWorkspace load protocol, access, time, and MapServer settings. |
...
| `Admin/AdminGitOpsWatchEndpoints:GET:/api/v{version:apiVersion}/admin/gitops/changes` | P1 | supported | /operator/publishing (PublishingWorkspace) and /operator/operations (OperationsConsole) | Publishing workspace and operations console render recent GitOps change history. |
